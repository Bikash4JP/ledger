...
# 4. 開発サーバーを起動（デモモードは .env 不要）
npx expo start -c
```

> **デモモード（バックエンド不要）：** ログインユーザーがいない状態では、56件の組み込み元帳とサンプル取引を使ったデモモードで自動的に起動します。UIを確認したい開発者はバックエンドなしで試せます。
>
> **ユーザーモード（クラウド同期）：** ログインとデータ同期を有効にするには、`.env.example` を `.env` にコピーしてバックエンドURLを設定してください：
> ```bash
> cp .env.example .env
> # .env を編集して EXPO_PUBLIC_API_URL にバックエンドサーバーのURLを設定
> ```
